...
For every retrieval session, first load `system.authority` and this policy by ID; load `system.frontmatter` when editing metadata. Resolve IDs through `_INDEX/entities.json`, open the owning Markdown, then follow only relations relevant to the question. Check status, authority, and `_INDEX/sync-status.json` before using a claim. Verify source symbols and lines against the hash-pinned files in `06_SOURCE/Code`. No comprehensive directional references or mixed E/lifecycle/bridge modules are retained. If a claim depends on them, report it as pending. A Vault snapshot does not prove that a later production checkout has the same bytes.
