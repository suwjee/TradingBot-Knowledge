...
For `type: mirror` notes only, `authority: canonical` identifies the curated direction-transformation contract grounded in the synchronized references and current source, with `status: active`. This is a knowledge-layer authority label, not a transfer of implementation ownership from source. Mirror notes use `source_reference` for verified production line anchors and `related_entities` for indexed `relates_to` edges. The other entity types retain their existing authority and status rules.
