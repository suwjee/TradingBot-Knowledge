...
A **Behavior** is the public semantic object and lifecycle role emitted by the pipeline. An **Algorithm** is the rule that calculates candidates, ownership, stops and output fields. One Behavior can have several calculation routes: for example, S Blue Type-1..4 share S's behavior family but use distinct routes. Reaction, Reset, Blue, Order and OrderAudit support calculation or evidence and do not become additional Behaviors. `test.behavior_invariants` validates this taxonomy against the serialized collections.
